...
const AlltypeProperties = () => {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `http://localhost:5000/api/v1/property?searchTerm=${searchTerm}&page=${page}&limit=${limit}`,
        {
          headers: {
  ...
  return (
    <div className="min-h-screen">
      <div className="">
        <PageHeader title={"All Property"}></PageHeader>
              ...
              {response?.data?.map((item,index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold ${item.isApproved ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
